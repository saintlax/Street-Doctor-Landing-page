    ...
    <div className="min-h-screen bg-emerald-50/30 font-sans text-slate-900">
      ...
      <nav className="flex items-center justify-between px-6 py-4 bg-white shadow-sm sticky top-0 z-50">
        ...
        <div className="hidden md:flex gap-8 font-medium">
          ...
          <a href="#features" className="hover:text-emerald-600 transition">AI Engine</a>
          <a href="#join" className="hover:text-emerald-600 transition">Join the Network</a>
        </div>
        <button 
          onClick={openWhatsApp}
          className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2 rounded-full font-semibold flex items-center gap-2 transition shadow-md"
        >
      ...
      <header className="px-6 py-16 md:py-24 max-w-6xl mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button 
            ...
            className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-4 rounded-xl font-bold text-lg transition shadow-lg shadow-emerald-200"
          >
            Start a Consultation
          </button>
          <button 
            onClick={comingSoonAlert}
            className="bg-white border-2 border-slate-200 hover:border-emerald-600 px-8 py-4 rounded-xl font-bold text-lg transition"
          >
            I am a Doctor
          </button>
        </div>
      ...
      <section className="bg-white py-12 border-y border-slate-200">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          <div>
            <div className="text-3xl font-bold text-emerald-600">50KM</div>
          ...
          <div>
            <div className="text-3xl font-bold text-emerald-600">AI-Verified</div>
      ...
      <section id="how-it-works" className="py-20 px-6 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {[
            { icon: <Watch className="text-emerald-500" />, title: "Sync", desc: "Wear your Street Doctor watch to track vitals 24/7." },
            { icon: <MapPin className="text-teal-500" />, title: "Triangulate", desc: "Share location via WhatsApp to find nearby doctors." },
            { icon: <Activity className="text-emerald-600" />, title: "Triage", desc: "AI combines your watch data with symptoms for a summary." },
            { icon: <CreditCard className="text-teal-600" />, title: "Secure Pay", desc: "Pay the consultation fee directly via chat." },
            { icon: <Zap className="text-emerald-400" />, title: "Resolve", desc: "Get a prescription or doorstep delivery instantly." }
          ].map((item, index) => (
            <div key={index} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-emerald-100 transition">
              <div className="mb-4 bg-emerald-50 w-12 h-12 flex items-center justify-center rounded-xl">{item.icon}</div>
      ...
      <section id="features" className="py-20 bg-slate-900 text-white px-6">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <div>
            <h2 className="text-3xl font-bold mb-6 italic text-emerald-400">The Street Doctor AI Engine</h2>
            <ul className="space-y-6">
              ...
              <li className="flex gap-4">
                <MessageSquare className="text-emerald-500 shrink-0" />
                <div>
          ...
          <div className="bg-slate-800 p-6 rounded-3xl border border-slate-700 shadow-2xl">
             ...
             <div className="space-y-4 font-mono text-sm">
               <div className="flex justify-between">
                 ...
                 <span className="text-emerald-400 flex items-center gap-1 animate-pulse"><Watch size={14}/> LIVE: 112 BPM</span>
               </div>
               <div className="text-emerald-100">SUMMARY: Patient reports acute abdominal pain. Geolocation: 2.4km away.</div>
               <div className="text-emerald-400">AI RECOMMENDATION: Rule out appendicitis. Watch data shows rising core temp.</div>
               <div className="bg-emerald-600 text-white p-2 rounded text-center mt-6 cursor-pointer hover:bg-emerald-500 transition">
      ...
      <section id="join" className="py-20 px-6 max-w-4xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-8 bg-emerald-50 rounded-2xl text-left border border-emerald-100">
            <h3 className="font-bold text-xl mb-4 text-emerald-800">For Independent Doctors</h3>
            <p className="text-slate-700 mb-6">Access live patient vitals, set your own fees, and let the AI handle your paperwork.</p>
            <button onClick={comingSoonAlert} className="flex items-center gap-2 font-bold text-emerald-600 hover:underline">Register Practice <ChevronRight size={16}/></button>
          </div>
